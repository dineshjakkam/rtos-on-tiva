...
uint32_t getlr(uint32_t x)
{
__asm(" LDRB R0, [R0,#-2]");
}

uint32_t getR(uint32_t x)
{
__asm(" LDR R0, [R0]");
}
...
k3=getR(j+8+40);
		...
		case 1:
			 i = 1,j=0;
			  bool found = false; bool ok = false;
			  ...
			  if (taskCount < MAX_TASKS)
			  {
			    ...
			    if (!found)
			    {
			      ...
			      tcb[i].priority = k3;
			      tcb[i].currentPriority = k3;
			      tcb[i].basePriority = k3;
			  ...
			  break;
		case 2:NVIC_ST_CTRL_R = NVIC_ST_CTRL_ENABLE | NVIC_ST_CTRL_INTEN | NVIC_ST_CTRL_CLK_SRC;
		 NVIC_ST_RELOAD_R = 0X9C68;
		  NVIC_INT_CTRL_R = NVIC_INT_CTRL_PEND_SV;
		  break;

		case 3: NVIC_INT_CTRL_R = NVIC_INT_CTRL_PEND_SV;
		  break;
...
void pendSVISR()
{
	...
	__asm(" POP {R3,lr}");
	__asm(" PUSH {R11,R10,R9,R8,R7,R6,R5,R4}");
...
SYSCTL_RCGC2_R |= SYSCTL_RCGC2_GPIOD | SYSCTL_RCGC2_GPIOE | SYSCTL_RCGC2_GPIOA |SYSCTL_RCGC2_GPIOF | SYSCTL_RCGC2_GPIOD;
...
void shell()
{
	while (true)
  {
	  bool found=false;int i=0,j;char printstring[10];char stc[4];uint64_t k=0;
	  ...
	  putsUart0("Enter Command:\r\n");
	  	  ...
	  	  if(isCommand(string)==true)
	  	  {
	  		  if(d==1)
	  		  {
	  			...
	  			k=k+tcb[j].timer;
	  			for(i=0;i<=taskCount;i++){
	  			tcb[i].cpuPercent=((tcb[i].timer*100)/k);
	  			tcb[i].timer=0;}
	  			  putsUart0("------------------------------------------------------------\r\n");
	  			 ...
	  			 for(i=1;i<=taskCount;i++)
	  			  {
	  				  sprintf(printstring,"%d		%s		%d",tcb[i].pid,tcb[i].name,tcb[i].cpuPercent);
